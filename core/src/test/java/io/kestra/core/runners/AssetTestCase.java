...
public class AssetTestCase {
    ...
    private static final List<Pair<AssetUser, AssetIdentifier>> capturedAssetLineage = new CopyOnWriteArrayList<>();
    ...
    public void staticAndDynamicAssets(String tenantId) throws QueueException, TimeoutException {
        ...
        Mockito.verify(mockedAssetService, Mockito.times(1)).eventListeners(Mockito.eq(Executor.class));

        List<TaskRun> taskRuns = execution.getTaskRunList().stream().toList();
        assertThat(taskRuns).map(TaskRun::getAssets).map(AssetsInOut::getInputs).satisfiesExactly(
            assets -> assertThat(assets).isEmpty(),
            assets -> assertThat(assets).isEmpty(),
            assets -> assertThat(assets).satisfiesExactly(
        ...
        assertThat(taskRuns).map(TaskRun::getAssets).map(AssetsInOut::getOutputs).satisfiesExactly(
            assets -> assertThat(assets).satisfiesExactly(
        ...
        assertThat(capturedAsyncCreate).satisfiesExactly(
            AssetTestCase::assertEnabledDynamicAsset,
            asset -> assertThat(asset.getId()).isEqualTo("assets-flow-static-emit-asset"),
            asset -> assertThat(asset.getId()).isEqualTo("assets-flow-static-asset-non-existing-output-uid"),
            asset -> assertThat(asset.getId()).isEqualTo("assets-flow-static-asset-existing-output-uid")
        ...
        assertThat(capturedAssetLineage).satisfiesExactly(
            asset -> {
                AssetUser assetUser = asset.getLeft();
                ...
                assertThat(asset.getRight().id()).isEqualTo("assets-flow-static-asset-non-existing-input-asset-uid");
            },
            asset -> {
                AssetUser assetUser = asset.getLeft();
                assertAssetExecution(tenantId, assetUser, execution);
                assertThat(assetUser.taskRunId()).isEqualTo(execution.getTaskRunList().stream().filter(taskRun -> taskRun.getTaskId().equals("static-asset-existing-input"))
                    .findFirst().map(TaskRun::getId).orElseThrow());
                assertThat(asset.getRight().id()).isEqualTo("assets-flow-static-asset-existing-input-uid");
            }
        );
    }
    ...
    static class MockFactory {
        ...
        public AssetService mockedAssetService() {
            return Mockito.spy(new AssetService() {
                ...
                @Override
                public void assetLineage(AssetUser assetUser, AssetIdentifier assetId) {
                    capturedAssetLineage.add(Pair.of(assetUser, assetId));
                }
            });
        }
